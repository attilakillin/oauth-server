...
class TokenTests {
    ...
    fun isExpired_withNoExpiration() {
        val now = Instant.now()
        Assertions.assertFalse(token(now, now, null).isExpired())
    }

    @Test
    fun isExpired_withFutureExpiration() {
        val now = Instant.now()
        Assertions.assertFalse(token(now, now, now.plusSeconds(delay)).isExpired())
    }

    @Test
    fun isExpired_withPastExpiration() {
        val now = Instant.now()
        Assertions.assertTrue(token(now, now, now.minusSeconds(delay)).isExpired())
    }

    @Test
    fun isTimestampValid_withAllCombinations() {
        forTimestamps { iat, nbe, exp, expected ->
            Assertions.assertEquals(expected, token(iat, nbe, exp).isTimestampValid())
        }
    }

    private fun token(iat: Instant, nbe: Instant, exp: Instant?): Token {
        return Token(
            value = "",
